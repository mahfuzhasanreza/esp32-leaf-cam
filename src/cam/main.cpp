...
// UART to ESP32 hub:
#define CAM_UART_BAUD   921600     // more reliable over longer wires
...
void setup() {
  ...
  // Warm-up: capture and discard one frame to stabilize sensor
  for (int i = 0; i < 1; ++i) {
  ...
  if (Serial.available()) {
    ...
    if (c == 'C') {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        if (!fb) break;
        // Drop the first buffered frame so the second fetch is the freshest
        if (i == 0) {
